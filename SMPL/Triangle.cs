...
namespace SMPL
{
	internal class Triangle
	{
		...
		public Triangle(Vertex p1, Vertex p2, Vertex p3, Image image, Vector3 normal, float normalZ)
		{
		...
		public Triangle[] GetZClippedTriangles()
		{
			...
			result.Push(new(vertsCamera[0], vertsCamera[1], vertsCamera[2], Image, normal, normalZ));
			...
			for (int i = 0; i < result.Count; i++)
			{
				...
				if (_out.Count == 0)
					result.Push(new(_in[0], _in[1], _in[2], Image, normal, normalZ));
				...
				else if (_out.Count == 1)
				{
					...
					{
						result.Push(new(extraPoint1, _in[0], _in[1], Image, normal, normalZ));
						result.Push(new(extraPoint2, extraPoint1, _in[1], Image, normal, normalZ));
					}
					else if (pointsAreOutside[1])
					{
						result.Push(new(extraPoint1, _in[1], _in[0], Image, normal, normalZ));
						result.Push(new(extraPoint2, extraPoint1, _in[0], Image, normal, normalZ));
					}
				...
				else if (_out.Count == 2)
				{
					...
					if (pointsAreOutside[0] == false)
						result.Push(new(extraPoint2, _in[0], extraPoint1, Image, normal, normalZ));
					else if (pointsAreOutside[1] == false)
						result.Push(new(extraPoint1, _in[0], extraPoint2, Image, normal, normalZ));
		...
		public Triangle[] GetClippedTriangles(Console console)
		{
			...
			for (int i = 0; i < result.Count; i++)
			{
				...
				else if (_out.Count == 2)
				{
					...
					result.Push(new(extraPoint1, extraPoint2, _in[0], Image, normal, normalZ));
				}
			}
			// right
			for (int i = 0; i < result.Count; i++)
